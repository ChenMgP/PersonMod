...
public class EChestEntity extends LootableContainerBlockEntity {
    private DefaultedList<ItemStack> inv = DefaultedList.ofSize(1, ItemStack.EMPTY);
    private String owner;

    public static void tick(EChestEntity entity, World world, BlockPos pos) {
        if (entity.inv.get(0).getItem() == ModItem.E_CHEST_KEY) {
            entity.inv.set(0, new ItemStack(Items.AIR));
            world.setBlockState(pos, ModBlock.E_OPEN_CHEST.getDefaultState());
    ...
    public EChestEntity(BlockPos blockPos, BlockState blockState) {
        super(ModBlockEntity.E_CHEST_BLOCK_ENTITY, blockPos, blockState);
    }
