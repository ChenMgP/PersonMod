...
public class CChestEntity extends LootableContainerBlockEntity {
    private DefaultedList<ItemStack> inv = DefaultedList.ofSize(1, ItemStack.EMPTY);
    private String owner;

    public static void tick(CChestEntity entity, World world, BlockPos pos) {
        if (entity.inv.get(0).getItem() == ModItem.C_CHEST_KEY) {
            entity.inv.set(0, new ItemStack(Items.AIR));
            world.setBlockState(pos, ModBlock.C_OPEN_CHEST.getDefaultState());
    ...
    public CChestEntity(BlockPos blockPos, BlockState blockState) {
        super(ModBlockEntity.C_CHEST_BLOCK_ENTITY, blockPos, blockState);
    }
