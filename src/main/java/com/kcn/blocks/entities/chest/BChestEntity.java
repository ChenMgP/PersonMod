...
public class BChestEntity extends LootableContainerBlockEntity {
    private DefaultedList<ItemStack> inv = DefaultedList.ofSize(1, ItemStack.EMPTY);
    private String owner;

    public static void tick(BChestEntity entity, World world, BlockPos pos) {
        if (entity.inv.get(0).getItem() == ModItem.B_CHEST_KEY) {
            entity.inv.set(0, new ItemStack(Items.AIR));
            world.setBlockState(pos, ModBlock.B_OPEN_CHEST.getDefaultState());
    ...
    public BChestEntity(BlockPos blockPos, BlockState blockState) {
        super(ModBlockEntity.B_CHEST_BLOCK_ENTITY, blockPos, blockState);
    }
