package com.kcn.blocks.cistern.jungle;
...
public class JungleCistern2 extends Block {
    public JungleCistern2(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (player.getMainHandStack().getItem() == ModItem.DELETE_SCROLL) {
            ...
            world.spawnEntity(new ItemEntity(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ModItem.JUNGLE_WOODEN_STAKE, 2)));
            world.playSound(pos.getX(), pos.getY(), pos.getZ(), SoundEvents.BLOCK_GLASS_BREAK, SoundCategory.BLOCKS, 4.0f, 1.75f, true);
            return ActionResult.SUCCESS;
        }
        if (player.getMainHandStack().getItem() == ModItem.JUNGLE_WOODEN_STAKE) {
            player.getMainHandStack().decrement(1);
            world.setBlockState(pos, ModBlock.JUNGLE_CISTERN_3.getDefaultState());
